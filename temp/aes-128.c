...
/*int aes_top_half(char* source_path, char* des_path, int size,int method,
		unsigned char* content,unsigned char* encry)
{
    char * password = "0f1571c947d9e8590cb7add6af7f6798";
    //将密钥转换成4*4数组
    int p[4][4];
    ...
    memcpy(content,source_path,size);
...
int aes_bottom_half(char* source_path, char* des_path, int size,int method,
		unsigned char* content,unsigned char* encry)
{
    char * password = "0f1571c947d9e8590cb7add6af7f6798";
    //将密钥转换成4*4数组
    ...
    for (int i = (size / 16)/2; i < (size / 16); ++i) {
...
int aes(char* source_path, char* des_path, int len,int method){
	...
	*/
    ...
    //读取文件内容赋值给content
    //fread(content, 1, len, file);
    memcpy(content,source_path,len);
